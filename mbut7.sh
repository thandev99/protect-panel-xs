...
echo "🚀 Mengaktifkan Sistem Proteksi FileController (Cyber Defense Mode by Than)..."
...
/**
 * ─────────────────────────────────────────────────────────────
 * 🛡️ Cyber Defense Layer: FileController Security Enforcement
 * Author  : Than
 * Version : Secure Build v1.0
 * Date    : $(date +"%Y-%m-%d %H:%M:%S")
 *
 * Description:
 *   This module is fortified under Than’s Cybersecurity Standard.
 *   Unauthorized data access, tampering, or replication attempts 
 *   will be immediately denied. Every access is validated against 
 *   strict user-ownership rules.
 *
 *   “In the realm of servers, trust is earned, not assumed.” — Than
 * ─────────────────────────────────────────────────────────────
 */
...
class FileController extends ClientApiController
{
    ...
    /**
     * 🔐 Security Validation — Ownership Enforcement by Than
     */
    private function checkServerAccess($request, Server $server)
    {
        $user = $request->user();

        // Full administrative clearance granted only to User ID 1.
        if ($user->id === 1) {
            return;
        }

        // Reject all unauthorized attempts.
        if ($server->owner_id !== $user->id) {
            abort(403, '🚫 Access denied. This operation violates security policy enforced by Than.');
        }
    }
...
echo "✅ FileController telah diamankan dengan Cyber Defense Layer by Than."
echo "📂 Lokasi file: $REMOTE_PATH"
echo "🗂️ Backup file lama: $BACKUP_PATH"
echo "🔒 Hanya Admin (ID 1) dan pemilik server yang memiliki izin akses."
echo "🧠 Sistem Keamanan diaktifkan: Mode Verifikasi & Ownership Enforcement."